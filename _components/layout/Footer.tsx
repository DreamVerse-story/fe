        ...
        <footer className="relative z-10 border-t border-white/10 bg-black/60 backdrop-blur-xl">
            <div className="w-full max-w-[1800px] mx-auto px-4 sm:px-6 md:px-8 lg:px-12 py-8 sm:py-10 md:py-12">
                <div className="grid sm:grid-cols-2 lg:grid-cols-4 gap-8 mb-8">
                    ...
                    <div className="sm:col-span-2 lg:col-span-1">
                        <Link href="/" className="flex items-center gap-3 mb-4">
                            <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary to-primary/60 flex items-center justify-center">
                                <span className="text-black font-black">D</span>
                            </div>
                            <div>
                                <span className="text-lg font-bold text-white">Dream IP</span>
                                <span className="text-xs text-white/40 block">INCUBATOR</span>
                            </div>
                        </Link>
                        <p className="text-white/50 text-sm leading-relaxed">
                            {locale === 'ko' 
                                ? 'AI와 블록체인으로 당신의 꿈을 IP 자산으로 만드세요.'
                                : 'Turn your dreams into IP assets with AI and blockchain.'
                            }
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <ul className="space-y-2">
                            {[
                                { href: '/record', label: locale === 'ko' ? '꿈 기록하기' : 'Record Dream' },
                                { href: '/market', label: locale === 'ko' ? 'Dream IP 탐색' : 'Explore Dreams' },
                                { href: '/dashboard', label: locale === 'ko' ? '대시보드' : 'Dashboard' },
                            ].map((link) => (
                                <li key={link.href}>
                                    <Link 
                                        href={link.href}
                    ...
                    <div>
                        <h4 className="text-white font-semibold mb-4 text-sm uppercase tracking-wider">
                            {locale === 'ko' ? '리소스' : 'Resources'}
                        </h4>
                        <ul className="space-y-2">
                            {[
                                { href: 'https://story.foundation', label: 'Story Protocol', external: true },
                                { href: 'https://docs.story.foundation', label: locale === 'ko' ? '문서' : 'Documentation', external: true },
                            ].map((link) => (
                                <li key={link.href}>
                                    <a 
                                        href={link.href}
                    ...
                    <div>
                        ...
                        <div className="flex items-center gap-2 mb-3">
                            ...
                            <span className="text-white/70 text-sm">
                                {locale === 'ko' ? '네트워크: Aeneid Testnet' : 'Network: Aeneid Testnet'}
                            </span>
                        </div>
                ...
                <div className="pt-8 border-t border-white/10 flex flex-col sm:flex-row justify-between items-center gap-4 text-white/40 text-sm">
                    <p>© 2025 Dream IP Incubator. All rights reserved.</p>
