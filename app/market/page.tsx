...
export default function SearchPage() {
    ...
    const loadDreams = async () => {
        try {
            ...
            if (data.success) {
                setDreams(
                    data.dreams.filter(
                        (d: DreamIPPackage) => d.status === 'completed'
                    )
                );
            }
        } catch (error) {
        ...
        return dreams
            .filter((dream) => {
                ...
                if (searchQuery) {
                    ...
                    const title = (dream.analysis.title || '').toLowerCase();
                    const titleEn = (dream.analysis.title_en || '').toLowerCase();
                    const summary = (dream.analysis.summary || '').toLowerCase();
                    const summaryEn = (dream.analysis.summary_en || '').toLowerCase();
                    
                    const matchesTitle = title.includes(query) || titleEn.includes(query);
                    const matchesDesc = summary.includes(query) || summaryEn.includes(query);
                    
                    // Search in arrays
                    const matchesGenres = (dream.analysis.genres || []).some(g => g.toLowerCase().includes(query));
                    const matchesTones = (dream.analysis.tones || []).some(t => t.toLowerCase().includes(query));
                    const matchesCharacters = (dream.analysis.characters || []).some(c => c.toLowerCase().includes(query));
                    
                    if (!matchesTitle && !matchesDesc && !matchesGenres && !matchesTones && !matchesCharacters) return false;
                }

                // Genre Filter
                if (selectedGenres.length > 0) {
                    const dreamGenres = dream.analysis.genres || [];
                    // Check if dream has ANY of the selected genres
                    const hasSelectedGenre = dreamGenres.some(genre => selectedGenres.includes(genre));
                    if (!hasSelectedGenre) return false;
                }

                // Tone Filter
                if (selectedTones.length > 0) {
                    const dreamTones = dream.analysis.tones || [];
                    // Check if dream has ANY of the selected tones
                    const hasSelectedTone = dreamTones.some(tone => selectedTones.includes(tone));
    ...
    }, [dreams, searchQuery, selectedGenres, selectedTones, sortBy]);

    // Unique values for filters
    const allGenres = useMemo(() => {
        const genres = new Set<string>();
        dreams.forEach(d => {
            (d.analysis.genres || []).forEach(g => genres.add(g));
        });
    ...
    const allTones = useMemo(() => {
        ...
        dreams.forEach(d => {
            (d.analysis.tones || []).forEach(t => tones.add(t));
        });
                ...
                <div className="flex flex-col lg:flex-row gap-8">
                    ...
                    <aside className="w-full lg:w-80 flex-shrink-0 space-y-8">
                        <div className="glass-panel rounded-2xl p-6 border border-white/10 bg-black/40 backdrop-blur-xl sticky top-32">
                            <div className="mb-8">
                                <h3 className="text-base font-bold text-white/90 uppercase tracking-widest mb-4">Search</h3>
                            ...
                            <div className="mb-8">
                                <h3 className="text-base font-bold text-white/90 uppercase tracking-widest mb-4">Genres</h3>
                                    ...
                                    {allGenres.map(genre => (
                                        <label key={genre} className="flex items-center gap-3 group cursor-pointer">
                                            <div className="relative flex items-center">
                                                ...
                                                />
                                                <svg className="pointer-events-none absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 w-3 h-3 text-black opacity-0 peer-checked:opacity-100 transition-opacity" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={3} d="M5 13l4 4L19 7" />
                                                </svg>
                                            </div>
                                            <span className="text-white/80 group-hover:text-white transition-colors text-base">{genre}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                            ...
                            <div>
                                <h3 className="text-base font-bold text-white/90 uppercase tracking-widest mb-4">Tones</h3>
                                <div className="space-y-2 max-h-48 overflow-y-auto pr-2 custom-scrollbar">
                                    {allTones.map(tone => (
                                        <label key={tone} className="flex items-center gap-3 group cursor-pointer">
                                            <div className="relative flex items-center">
                                                <input
                                                    type="checkbox"
                                            ...
                                            <span className="text-white/80 group-hover:text-white transition-colors text-base">{tone}</span>
                                        </label>
                                    ))}
                                </div>
                            </div>
                    ...
                    <div className="flex-1">
                        <div className="flex justify-between items-center mb-8">
                            <h2 className="text-2xl font-bold text-white">
                                {filteredDreams.length} <span className="text-white/90 text-xl font-normal">Results Found</span>
                            </h2>
                        ...
                        {filteredDreams.length > 0 ? (
                            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
                                {filteredDreams.map((dream) => (
                                    <div key={dream.id} className="transform hover:-translate-y-2 transition-transform duration-300">
                                        <DreamIPCard
                                            dream={dream}
                            ...
                            <div className="glass-panel rounded-2xl p-16 text-center border-dashed border-2 border-white/10 bg-black/20">
                                ...
                                <h3 className="text-xl font-bold text-white mb-2">No Dreams Found</h3>
                                <p className="text-white/90 text-lg">Try adjusting your filters or search query.</p>
                            </div>
                        )}
                    </div>
                </div>
