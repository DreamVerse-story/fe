...
export default function DashboardPage() {
    ...
    const [allDreams, setAllDreams] = useState<DreamIPPackage[]>([]);
    ...
    // 통계 계산
    const stats = useMemo(() => {
        const myMinted = dreams.filter((d: any) => d.ipAssetId).length;
        const myPending = dreams.filter((d: any) => !d.ipAssetId && d.status === 'completed').length;
        const totalMinted = allDreams.filter((d: any) => d.ipAssetId).length;
    ...
    const tabs = [
        { id: 'overview' as TabType, label: locale === 'ko' ? '개요' : 'Overview', icon: '📊' },
        { id: 'dreams' as TabType, label: locale === 'ko' ? '내 꿈' : 'My Dreams', icon: '🌙' },
        { id: 'royalties' as TabType, label: locale === 'ko' ? '로열티' : 'Royalties', icon: '💎' },
    ];
        ...
        <PageContainer showBackground={true} backgroundType="default">
            <div className="max-w-7xl mx-auto space-y-8">
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
                    ...
                    {isConnected && (
                        <Button variant="primary" onClick={() => router.push('/record')}>
                            <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 4v16m8-8H4" />
                            </svg>
                            {locale === 'ko' ? '새 꿈 기록' : 'Record Dream'}
                        </Button>
                    )}
                </div>
...
function WalletRequiredState({ locale }: { locale: string }) {
    return (
...
function OverviewTab({ stats, dreams, locale, router }: { 
    stats: any; 
    ...
    const pendingDreams = dreams.filter((d: any) => !d.ipAssetId && d.status === 'completed');
    ...
    return (
        <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                <StatCard 
                    label={locale === 'ko' ? '총 Dream IP' : 'Total Dreams'} 
                    value={stats.total} 
                    icon="🌙"
                    color="primary"
                />
                <StatCard 
                    label={locale === 'ko' ? '민팅 완료' : 'Minted'} 
                    value={stats.minted} 
                    icon="✨"
                    color="secondary"
                />
                <StatCard 
                    label={locale === 'ko' ? '민팅 대기' : 'Pending Mint'} 
                    value={stats.pending} 
                    icon="⏳"
                    color="warning"
                />
                <StatCard 
                    label={locale === 'ko' ? '플랫폼 전체' : 'Platform Total'} 
            ...
            {pendingDreams.length > 0 && (
                <Card variant="glass" padding="md" className="border-yellow-500/30 bg-yellow-500/5">
                    <div className="flex items-center justify-between gap-4 flex-wrap">
                        ...
                        <Button 
                            variant="primary" 
                            size="sm"
                            onClick={() => router.push(`/dreams/${pendingDreams[0].id}?new=true`)}
                        >
                            {locale === 'ko' ? '지금 민팅하기' : 'Mint Now'}
                        </Button>
                    </div>
                </Card>
            )}
            ...
            <div>
                <div className="flex items-center justify-between mb-4">
                    <h2 className="text-xl font-bold text-white">
                        {locale === 'ko' ? '최근 Dream IP' : 'Recent Dreams'}
                    </h2>
                    {dreams.length > 3 && (
                        <button 
                            onClick={() => {}} 
                ...
                {recentDreams.length > 0 ? (
                    <div className="grid gap-4">
                        {recentDreams.map((dream) => (
                            <DreamListItem key={dream.id} dream={dream} locale={locale} router={router} />
                        ))}
                    </div>
                ) : (
                    <Card variant="glass" padding="lg" className="text-center">
                        <p className="text-white/60">
                            {locale === 'ko' ? '아직 기록한 꿈이 없습니다.' : 'No dreams recorded yet.'}
                        </p>
                        <Button 
                            variant="ghost" 
                            className="mt-4"
                        ...
                        >
                            {locale === 'ko' ? '첫 번째 꿈 기록하기' : 'Record Your First Dream'}
                        </Button>
                    </Card>
                )}
            </div>
...
function DreamsTab({ dreams, locale, router }: { 
    dreams: DreamIPPackage[]; 
    locale: string;
    router: any;
}) {
    const [filter, setFilter] = useState<'all' | 'minted' | 'pending'>('all');
    
    const filteredDreams = useMemo(() => {
        switch (filter) {
            case 'minted':
                return dreams.filter((d: any) => d.ipAssetId);
            case 'pending':
                return dreams.filter((d: any) => !d.ipAssetId && d.status === 'completed');
            default:
                return dreams;
        }
    }, [dreams, filter]);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="flex gap-2">
                {[
                    { id: 'all' as const, label: locale === 'ko' ? '전체' : 'All', count: dreams.length },
                    { id: 'minted' as const, label: locale === 'ko' ? '민팅됨' : 'Minted', count: dreams.filter((d: any) => d.ipAssetId).length },
                    { id: 'pending' as const, label: locale === 'ko' ? '대기중' : 'Pending', count: dreams.filter((d: any) => !d.ipAssetId && d.status === 'completed').length },
                ].map((f) => (
                    <button
                        key={f.id}
            ...
            {filteredDreams.length > 0 ? (
                <div className="grid gap-4">
                    {filteredDreams.map((dream) => (
                        <DreamListItem key={dream.id} dream={dream} locale={locale} router={router} expanded />
                    ))}
                </div>
            ) : (
                <Card variant="glass" padding="lg" className="text-center">
                    <p className="text-white/60">
                        {locale === 'ko' ? '해당하는 꿈이 없습니다.' : 'No dreams found.'}
                    </p>
                </Card>
            )}
        </div>
...
function RoyaltiesTab({ dreams, locale }: { 
    dreams: DreamIPPackage[]; 
    locale: string;
}) {
    const mintedDreams = dreams.filter((d: any) => d.ipAssetId);
    ...
    return (
        <div className="space-y-6">
            ...
            <div className="grid sm:grid-cols-3 gap-4">
                <StatCard 
                    label={locale === 'ko' ? '총 수익' : 'Total Earnings'} 
                    value="0 IP" 
                    icon="💰"
                    color="primary"
                    isText
                />
                <StatCard 
                    label={locale === 'ko' ? '청구 가능' : 'Claimable'} 
                    value="0 IP" 
                    icon="📥"
                    color="secondary"
                    isText
                />
                <StatCard 
                    label={locale === 'ko' ? '라이선스 판매' : 'Licenses Sold'} 
                    value="0" 
                    icon="📜"
                    color="accent"
                />
                                ...
                                <div className="flex items-center gap-3">
                                    ...
                                    <div>
                                        <h4 className="font-semibold text-white">
                                            {locale === 'en' && dream.analysis.title_en 
                                                ? dream.analysis.title_en 
                                                : dream.analysis.title}
                                        </h4>
                                        <p className="text-xs text-white/40">
                                            {locale === 'ko' ? '라이선스 판매: 0건' : 'Licenses sold: 0'}
                                        </p>
                                    </div>
                                </div>
                                <div className="text-right">
                                    <p className="text-primary font-bold">0 IP</p>
                                    <p className="text-xs text-white/40">
                                        {locale === 'ko' ? '청구 가능' : 'Claimable'}
                                    </p>
                                </div>
                            </Card>
                        ))}
...
function DreamListItem({ dream, locale, router, expanded = false }: { 
            ...
            <div className="flex items-center gap-4 p-4">
                ...
                <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                        ...
                        <Badge variant={isMinted ? 'success' : 'warning'} className="shrink-0">
                            {isMinted 
                ...
                {!isMinted && (
                    <Button 
                        ...
                        onClick={(e) => {
                            e.stopPropagation();
                            router.push(`/dreams/${dream.id}?new=true`);
                        }}
